-- begin DDCIL_USER_INPUT_PARAMETER
alter table DDCIL_USER_INPUT_PARAMETER add constraint FK_DDCIL_USER_INPUT_PARAMETER_LAUNCHER_COMMAND foreign key (LAUNCHER_COMMAND_ID) references DDCIL_LAUNCHER_CMD(ID)^
create index IDX_DDCIL_USER_INPUT_PARAMETER_LAUNCHER_COMMAND on DDCIL_USER_INPUT_PARAMETER (LAUNCHER_COMMAND_ID)^
-- end DDCIL_USER_INPUT_PARAMETER
-- begin DDCIL_LAUNCHER_CMD_TRANSLATION
alter table DDCIL_LAUNCHER_CMD_TRANSLATION add constraint FK_DDCIL_LAUNCHER_CMD_TRANSLATION_LAUNCHER_COMMAND foreign key (LAUNCHER_COMMAND_ID) references DDCIL_LAUNCHER_CMD(ID)^
create index IDX_DDCIL_LAUNCHER_CMD_TRANSLATION_LAUNCHER_COMMAND on DDCIL_LAUNCHER_CMD_TRANSLATION (LAUNCHER_COMMAND_ID)^
create index IDX_DDCIL_LAUNCHER_COMMAND_TRANSLATION on DDCIL_LAUNCHER_CMD_TRANSLATION (LOCALE_, TEXT) ^
-- end DDCIL_LAUNCHER_CMD_TRANSLATION
-- begin DDCIL_INPUT_PARAMETER_TRANSLATION
alter table DDCIL_INPUT_PARAMETER_TRANSLATION add constraint FK_DDCIL_INPUT_PARAMETER_TRANSLATION_INPUT_PARAMETER foreign key (INPUT_PARAMETER_ID) references DDCIL_USER_INPUT_PARAMETER(ID)^
create index IDX_DDCIL_INPUT_PARAMETER_TRANSLATION_INPUT_PARAMETER on DDCIL_INPUT_PARAMETER_TRANSLATION (INPUT_PARAMETER_ID)^
-- end DDCIL_INPUT_PARAMETER_TRANSLATION
-- begin DDCIL_LAUNCHER_CMD
alter table DDCIL_LAUNCHER_CMD add constraint FK_DDCIL_LAUNCHER_CMD_GROUP foreign key (GROUP_ID) references DDCIL_LAUNCHER_COMMAND_GROUP(ID)^
create index IDX_DDCIL_LAUNCHER_CMD_GROUP on DDCIL_LAUNCHER_CMD (GROUP_ID)^
-- end DDCIL_LAUNCHER_CMD
-- begin DDCIL_SCREEN_LAUNCHER_CMD
alter table DDCIL_SCREEN_LAUNCHER_CMD add constraint FK_DDCIL_SCREEN_LAUNCHER_CMD_ID foreign key (ID) references DDCIL_LAUNCHER_CMD(ID)^
-- end DDCIL_SCREEN_LAUNCHER_CMD
-- begin DDCIL_SCRIPT_LAUNCHER_CMD
alter table DDCIL_SCRIPT_LAUNCHER_CMD add constraint FK_DDCIL_SCRIPT_LAUNCHER_CMD_ID foreign key (ID) references DDCIL_LAUNCHER_CMD(ID)^
-- end DDCIL_SCRIPT_LAUNCHER_CMD
-- begin DDCIL_BEAN_LAUNCHER_COMMAND
alter table DDCIL_BEAN_LAUNCHER_COMMAND add constraint FK_DDCIL_BEAN_LAUNCHER_COMMAND_ID foreign key (ID) references DDCIL_LAUNCHER_CMD(ID)^
-- end DDCIL_BEAN_LAUNCHER_COMMAND
-- begin DDCIL_LAUNCHER_COMMAND_GROUP_TRANSLATION
alter table DDCIL_LAUNCHER_COMMAND_GROUP_TRANSLATION add constraint FK_DDCIL_LCGT_LAUNCHER_COMMAND_GROUP foreign key (LAUNCHER_COMMAND_GROUP_ID) references DDCIL_LAUNCHER_COMMAND_GROUP(ID)^
create index IDX_DDCIL_LCGT_LAUNCHER_COMMAND_GROUP on DDCIL_LAUNCHER_COMMAND_GROUP_TRANSLATION (LAUNCHER_COMMAND_GROUP_ID)^
-- end DDCIL_LAUNCHER_COMMAND_GROUP_TRANSLATION
